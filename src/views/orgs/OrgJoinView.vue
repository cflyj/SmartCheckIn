...
<template>
  <div class="page">
    ...
    <div class="content">
      <p v-if="phase === 'input'" class="muted" style="font-size: 14px; line-height: 1.45">
      ...
      <template v-else-if="preview?.ambiguous">
        <p class="muted" style="font-size: 14px; line-height: 1.45; margin-top: 0">
          该加入码对应<strong>多个组织</strong>，请根据负责人告知的名称点选正确的一个，避免加错群。
        </p>
        <p class="muted" style="font-size: 13px">新建组织时系统会避免与他人重复加入码；若仍出现多条，多为历史数据，请务必核对名称。</p>
        <div class="grouped-list" style="margin-bottom: 16px">
          <button
            v-for="c in preview.candidates"
            :key="c.id"
            type="button"
            class="list-cell"
            style="display: block; width: 100%; text-align: left; cursor: pointer; border: none; background: var(--cell-bg, #f2f2f7); margin: 0 0 8px; border-radius: 10px; padding: 12px 14px"
            :style="
              selectedOrgId === c.id
                ? { boxShadow: '0 0 0 2px var(--accent, #007aff)' }
                : {}
            "
            @click="selectedOrgId = c.id"
          >
            <div class="list-cell__title">{{ c.name }}</div>
            <div class="muted" style="font-size: 13px; margin-top: 4px">{{ policyLabel(c.join_policy) }}</div>
          </button>
        </div>

        <template v-if="activeTarget">
          <div class="card card-pad" style="margin-bottom: 16px">
            <p class="list-cell__title" style="margin-bottom: 6px">{{ activeTarget.name }}</p>
            <p class="muted" style="font-size: 14px; margin: 0 0 8px">{{ policyLabel(activeTarget.join_policy) }}</p>
            <p class="muted" style="font-size: 14px; margin: 0; line-height: 1.45">{{ policyDetail(activeTarget.join_policy) }}</p>
          </div>

          <template v-if="activeTarget.already_member">
            <p class="muted" style="font-size: 14px">你已是该组织成员。</p>
          ...
          <template v-else-if="activeTarget.has_pending_request">
            <p class="muted" style="font-size: 14px">你已提交入组申请，请等待管理员审核。</p>
            <button type="button" class="btn btn-primary" @click="router.replace({ name: 'orgs' })">返回组织列表</button>
          </template>
          <template v-else-if="activeTarget.join_policy === 'invite_only'">
            <p class="muted" style="font-size: 14px; line-height: 1.45; margin: 0 0 12px">
              把你在本系统注册的<strong>用户名</strong>告诉组织管理员，请对方在「组织详情 → 添加成员」里加你。
            </p>
            <button type="button" class="btn btn-primary" @click="router.replace({ name: 'orgs' })">返回组织列表</button>
            <button type="button" class="btn btn-secondary" style="margin-top: 10px; display: block; width: 100%" @click="backToInput">
              换加入码重试
            </button>
          </template>
          <template v-else>
            ...
            <button type="button" class="btn btn-secondary" style="margin-top: 10px" :disabled="loading" @click="backToInput">
              返回修改加入码
            </button>
          </template>
        </template>
        <p v-else class="muted" style="font-size: 14px">请先在上方选择一个组织。</p>
      </template>

      <template v-else-if="preview && !preview.ambiguous && preview.org">
        <div class="card card-pad" style="margin-bottom: 16px">
          <p class="list-cell__title" style="margin-bottom: 6px">{{ preview.org.name }}</p>
          <p class="muted" style="font-size: 14px; margin: 0 0 8px">{{ policyLabel(preview.org.join_policy) }}</p>
          <p class="muted" style="font-size: 14px; margin: 0; line-height: 1.45">{{ policyDetail(preview.org.join_policy) }}</p>
        </div>
