...
import {
  GEO_LOCATE_HARD_MAX_MS,
  GEO_LOCATE_HARD_MAX_SEC,
  GEO_LOCATE_MIN_BEFORE_EARLY_MS,
  GEO_LOCATE_GOOD_ACCURACY_M,
  GEO_LOCATE_STALL_MS,
...
const hasGeo = computed(() =>
  session.value
    ? ['GEO', 'BOTH', 'GEO_FACE', 'GEO_QR_FACE'].includes(session.value.checkin_modes)
    : false
)
const hasQr = computed(() =>
  session.value ? ['QR', 'BOTH', 'GEO_QR_FACE'].includes(session.value.checkin_modes) : false
)

const geoCardVisible = computed(() => {
  const s = session.value
  if (!s || !hasGeo.value) return false
  const m = s.checkin_modes
  if (m === 'BOTH' || m === 'GEO_QR_FACE') return tab.value === 'geo'
  return m === 'GEO' || m === 'GEO_FACE'
})

const qrCardVisible = computed(() => {
  const s = session.value
  if (!s || !hasQr.value) return false
  const m = s.checkin_modes
  if (m === 'BOTH' || m === 'GEO_QR_FACE') return tab.value === 'qr'
  return m === 'QR'
})

const showFaceOnlyPanel = computed(
  () => checkinMode.value === 'FACE' || (checkinMode.value === 'GEO_QR_FACE' && tab.value === 'face')
)
...
const useStandaloneGeoSubmit = computed(() => {
  ...
  if (m === 'GEO_QR_FACE' && tab.value === 'geo') return true
  return false
})
...
<template>
  <AppPageShell :nav-title="session?.title || '活动'" @back="router.push({ name: 'participant-sessions' })">
    ...
    <div class="content stack stack--md stack--airy">
      ...
      <template v-else-if="session">
        ...
        <div v-if="session.checkin_modes === 'GEO_QR_FACE'" class="tabs">
          <button type="button" :class="['tab', tab === 'geo' && 'tab--active']" @click="tab = 'geo'">
            地理位置
          </button>
          <button type="button" :class="['tab', tab === 'qr' && 'tab--active']" @click="tab = 'qr'">
            二维码
          </button>
          <button type="button" :class="['tab', tab === 'face' && 'tab--active']" @click="tab = 'face'">
            人脸识别
          </button>
        </div>
